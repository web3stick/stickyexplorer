...
pub fn render_ft_transfer(tx: &TransactionDetail) -> Element {
    ...
    rsx! {
        div { class: "mb-4 rounded-lg border {status_class}",
...
pub fn match_near_transfer(tx: &TransactionDetail) -> bool {
    ...
    parsed.actions.len() == 1
        && parsed
            .actions
            .first()
            .map_or(false, |a| a.action_type == "Transfer")
        && parsed
            .actions
            .first()
            .and_then(|a| a.deposit.as_ref())
            .map_or(false, |d| d != "0")
}
...
pub fn render_default_widget(tx: &TransactionDetail) -> Element {
    ...
    rsx! {
        div { class: "mt-6 rounded-lg border border-gray-200 bg-white",
            button { onclick: move |_| open.toggle(), class: "json-toggle-button",
                span { "Raw JSON" }
            }
            if open() {
